...
class FixedCommandManager(CommandManagerInterface):
    ...
    def __init__(self, command=np.zeros(10)):
...
class Arm:
    def __init__(self, log_file: h5py.File = None, prev_steps=3, command_manager=None):
    ...
    def update(self):
        ...
        self.latency = (datetime.datetime.now() - self._arm.timestamp).total_seconds()
        self.timestamp = time.perf_counter()
    ...
    def step(self, action=None):
        ...
        if action is not None:
            ...
            self.last_action = self.last_action * 0.2 + action * 0.8
            target = (self.last_action + self.default_joint_pos).clip(-np.pi, np.pi)
            target = np.concatenate([target, [0]]).tolist()
...
def main():
    ...
    robot = Arm(log_file, command_manager=KeyboardCommandManager())
    ...
    try:
        ...
        with torch.inference_mode(), set_exploration_type(ExplorationType.MODE):
            for i in itertools.count():
                ...
                cmd, obs = torch.as_tensor(robot.step(action))
